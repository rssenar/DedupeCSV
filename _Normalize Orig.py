...
# ---------------------------------------------
os.chdir('../../../../Desktop/')
rpath = '../Dropbox/HUB/Projects/_Resources'
# ---------------------------------------------
HRSelect = 'Y'
# ---------------------------------------------
...
# ---------------------------------------------
ZipCoordFile = os.path.join(rpath,'US_ZIP_Coordinates.csv')
YearDecodeFile = os.path.join(rpath,'Year_Decode.csv')
GenSuppressionFile = os.path.join(rpath,'GEN_Suppression.csv')
MonthlySuppressionFile = os.path.join(rpath,'Monthly_Suppression.csv')
DropFile = os.path.join(rpath,'Drop_File.csv')
# ---------------------------------------------
WinningNumber = 40754
# ---------------------------------------------
...
CleanOutput1 = IPFName + '_OutputMASTER.csv'
# ---------------------------------------------
...
AppendMonthlySuppFile = '>>> ' + IPFName + '_Append Monthly Suppression.csv'
# ---------------------------------------------
...
def ReMapHeaderFields():
	...
	def match(field):
		...
		elif bool(re.search(r'\byeardec\b',field,flags=re.I)):
...
def SetDropIndex():
	if line[DSF_WALK_SEQ] == '' and line[Zip] in DropDict:
		line[DropVal] = DropDict[line[Zip]]
	elif line[DSF_WALK_SEQ] != '' and line[ZipCRRT] in DropDict:
...
def YearDecode():
	if str(line[Year]) in YearDecodeDict:
		line[YearDec] = YearDecodeDict[line[Year]]
	else:
		line[YearDec] = ''
...
def CombineAddress(): 
	if line[AddressComb] == '' and line[Address1] != '' and line[Address2] != '':
		line[AddressComb] = str.title(line[Address1]) + ' ' + str.title(line[Address2])
	elif line[AddressComb] == '' and line[Address1] != '' and line[Address2] == '':
...
# ---------------------------------------------
try:
	ProcDataFrame = pd.read_csv(CleanOutput1, low_memory = False)
	ProcDataF = ProcDataFrame.loc[:,['Radius']]
	print(ProcDataF.describe())
except:
	print('Output File Empty!')
# ---------------------------------------------
print('Data Processed & Normailized!')
# ---------------------------------------------
